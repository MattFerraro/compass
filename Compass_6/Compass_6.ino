...
- any radio packet received prior to getting GPS time may be given an incorrect incoming timestamp
...
- if no fix is made in BG2, the GPS is being turned off anyway (I think).  Might
be better to just go back to BG1
- the first time the toggle light button is pressed, it doesn't seem to do anything
  ...
  while (Serial1.available()) {
    ...
    if (Serial1.read() == '[') { // If the packet has started
      ...
      if (radio_error == 0) { // Do this if there are enough bytes and no error
        byte user_bytes[8]; // temp storage for incoming username
        byte radio_pos_data[16]; // temp storage for incoming lat/lon floats
        for (byte t = 0; t < 8; t++) { // Read the username
          user_bytes[t] = Serial1.read(); // ... byte by byte
        }
        Serial.print("radio data: ");
        for (byte t = 0; t < 16; t++) { // read in all lat/long data
          radio_pos_data[t] = Serial1.read();
          Serial.write(radio_pos_data[t]);
        }
        Serial.println();
        ...
        for (byte t = 0; t < 4; t++) {
          lat_bytes[-1 * t + 3] = 16 * hex_str(radio_pos_data[2 * t]) + hex_str(radio_pos_data[2 * t + 1]);
          lon_bytes[-1 * t + 3] = 16 * hex_str(radio_pos_data[2 * t + 8]) + hex_str(radio_pos_data[2 * t + 9]);
        }
        byte incoming_user_index = 255; // 255 will be the null value here
        // figure out which username was just read in
        String incoming_user_string = String(user_bytes);
        for (byte t; t<sizeof(user_names); t++){
          String user_list_string = String(user_names[t]);
          if (incoming_user_string == user_list_string){
            incoming_user_index = t;
          }
        }

        Serial.print("incoming_user_index is: ");
        Serial.println(incoming_user_index);

        // ------------ GOT TO HERE
        // next step is to make sure the incoming user index is being set right
        // by transmitting user packets to a test unit
...
void menu_display(byte menu_location_id, int menu_map[][4], char* menu_labels[], byte menu_map_size) {
  ...
  if (prev_id >= 0) {
    ...
    String label_string = "  " + String(menu_labels[prev_id]);
    label_string.toCharArray(prev_id_text, sizeof(prev_id_text));

    full_word(2, prev_id_text);
  }

  String label_string = "> " + String(menu_labels[menu_location_id]); // add bullet triangle > to this one
  label_string.toCharArray(current_id_text, sizeof(current_id_text));
  ...
  if (next_id >= 0) {
    ...
    String label_string = "  " + String(menu_labels[next_id]);
    label_string.toCharArray(next_id_text, sizeof(next_id_text));
